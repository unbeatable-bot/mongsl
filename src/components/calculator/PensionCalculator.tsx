  ...
  return (
    ...
    <div className="bg-main-bg p-[24px] rounded-[16px] border border-border-color flex flex-col h-full hover:border-[#444] transition-colors duration-300 w-full">
      <div className="mb-[20px]">
        ...
        {/* ✨ 텍스트 색상 적용 */}
        <p className="text-main-text/60 text-[13px] m-0 leading-relaxed">{!isReverseMode ? '모아둔 은퇴 자산으로 매월 얼마씩 수령할 수 있는지 계산합니다.' : '희망하는 월 수령액을 받기 위해 지금 당장 필요한 은퇴 자산을 역계산합니다.'}</p>
      </div>

      <div className="mb-[15px]">
        <label className="block text-[12px] text-main-text/80 mb-[5px]">{!isReverseMode ? '보유한 총 자산 (연금 총액)' : '은퇴 후 희망 월 수령액'}</label>
      ...
      <div className="flex gap-[8px] flex-wrap mb-[30px]">
        {/* ✨ 퀵 버튼 테마 적용 */}
        <button onClick={() => addAmount(!isReverseMode ? 10000000 : 100000)} className="px-[14px] py-[6px] rounded-full bg-sub-bg border border-border-color text-main-text/80 hover:border-[#52c41a] hover:text-[#52c41a] text-[12px] transition-colors">{!isReverseMode ? '+1천만' : '+10만'}</button>
        <button onClick={() => addAmount(!isReverseMode ? 100000000 : 1000000)} className="px-[14px] py-[6px] rounded-full bg-sub-bg border border-border-color text-main-text/80 hover:border-[#52c41a] hover:text-[#52c41a] text-[12px] transition-colors">{!isReverseMode ? '+1억' : '+100만'}</button>
        <button onClick={() => addAmount(!isReverseMode ? 1000000000 : 10000000)} className="px-[14px] py-[6px] rounded-full bg-sub-bg border border-border-color text-main-text/80 hover:border-[#52c41a] hover:text-[#52c41a] text-[12px] transition-colors">{!isReverseMode ? '+10억' : '+1000만'}</button>
      </div>

      <div className="grid grid-cols-2 gap-[20px] mb-[30px]">
        <div>
          <label className="block text-[12px] text-main-text/80 mb-[5px]">수령 기간 (년)</label>
        ...
        <div>
          <label className="block text-[12px] text-main-text/80 mb-[5px]">은퇴 후 기대수익률 (%)</label>
      ...
      <div className="mt-auto pt-[20px] border-t border-border-color flex flex-col gap-[10px]">
        <div className="flex justify-between items-end text-main-text">
          <span className="text-main-text/60 text-[14px]">총 수령액 / 수령 중 굴러간 이자</span>
          <span className="text-main-text/90 font-bold text-[16px]">{formatComma(totalReceived.toFixed(0))} 원 / <span className="text-[#52c41a]">+{formatComma(totalInterest.toFixed(0))} 원</span></span>
        </div>
        <div className="flex justify-between items-end text-main-text mt-[10px] pt-[10px] border-t border-border-color border-dashed">
          <span className="font-bold text-main-text text-[18px]">{!isReverseMode ? '매월 예상 수령액' : '지금 필요한 총 자산'}</span>
