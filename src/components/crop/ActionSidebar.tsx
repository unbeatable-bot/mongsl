  ...
  return (
    <div className="flex flex-col items-stretch justify-start gap-[10px] ml-[15px] shrink-0 max-md:flex-row max-md:flex-wrap max-md:justify-between max-md:ml-0 max-md:w-full">
      ...
      <div className="flex flex-col gap-[5px] bg-[#2a2a2a] p-[10px] rounded-[5px] border border-[#444] mt-[5px] max-md:w-full max-md:flex-[1_1_100%]">
        <label className="text-[12px] text-[#bdc3c7]">압축 품질 (용량)</label>
        <select value={quality} onChange={(e) => setQuality(Number(e.target.value))} disabled={isProcessing} className="bg-[#1e1e1e] text-white border border-[#555] p-[6px] rounded-[4px] text-[13px] cursor-pointer">
      ...
      <button onClick={() => openModal(<InfoPage onClose={closeModal} />)} disabled={isProcessing} className={`bg-[#6c757d] hover:bg-[#5c636a] flex justify-center items-center gap-[4px] ${btnClass}`}><span className="text-[16px]">❓</span>사용 안내</button>
    </div>
  );
};
